...
type ShallowStorage struct {
	...
	// mu guards the cache fields and, in SetShallow, the on-disk write
	// itself: SetShallow holds it for the whole open-write-close-and-cache
	// sequence, not just the final cache update, so a concurrent
	// Shallow/IsShallow can never observe the file mid-write (e.g.
	// truncated by ShallowWriter but not yet rewritten) and cache that
	// torn read as though it were final. Without that, a reader racing a
	// SetShallow that goes on to fail at Close would cache the truncated
	// file's contents and never see it corrected, since a failed
	// SetShallow does not update the cache. The shallow file has no other
	// writer in this module (nothing removes it, and this is the only call
	// to dotgit.ShallowWriter), so the cache can otherwise only go stale
	// against another process, which won't be seen until this Storage is
	// reopened.
	//
...
func (s *ShallowStorage) SetShallow(commits []plumbing.Hash) (err error) {
	...
	defer func() {
		if err == nil {
			s.setCacheLocked(slices.Clone(commits))
		}
	}()
