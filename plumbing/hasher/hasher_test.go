...
func BenchmarkCompute(b *testing.B) {
	content := []byte(strings.Repeat("s", 5000))

	b.Run("plumbing.compute-sha1", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			b.SetBytes(int64(len(content)))
			plumbing.ComputeHash(plumbing.AnyObject, content)
		}
	})

	oh, err := FromObjectFormat(config.SHA1)
	require.NoError(b, err)
	b.Run("hasher.compute-sha1", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			b.SetBytes(int64(len(content)))
			oh.Compute(plumbing.AnyObject, content)
		}
	})

	oh, err = FromObjectFormat(config.SHA256)
	require.NoError(b, err)
	b.Run("hasher.compute-sha256", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			b.SetBytes(int64(len(content)))
			oh.Compute(plumbing.AnyObject, content)
		}
