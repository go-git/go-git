...
type CreateCommitNodeGraphOptions struct {
	Commits object.CommitIter // If nil then all commits in the storage are used as potential heads, otherwise only the commits in the iter are used
	Append  bool              // Append all commits in the old index to the new index
	Chain   bool              // Chain the new index to the old index (only valid if Append is false and the old index has generation v2)
}
