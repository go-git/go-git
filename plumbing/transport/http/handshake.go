...
func handshakeSmart(resp *http.Response, req *transport.Request, client *http.Client, authorizer func(*http.Request) error) (transport.Session, error) {
	...
	case protocol.V2:
		...
		if err := caps.DecodeList(rd); err != nil {
			return nil, err
		}
		runner := &httpRunner{
			client:     client,
		...
		return transport.NewV2Session(runner, caps, req.Command, true), nil
...
func (r *httpRequester) doPost() error {
	...
	if r.resp.StatusCode != http.StatusOK {
		_ = r.resp.Body.Close()
		return fmt.Errorf("http transport: POST %s unexpected status %d", redactedURL(httpReq.URL), r.resp.StatusCode)
	}
	return nil
}
