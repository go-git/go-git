...
func TestFetchOutputDecode(t *testing.T) {
	...
	t.Run("packfile reader is live", func(t *testing.T) {
		...
		got := &FetchOutput{}
		require.NoError(t, got.Decode(&buf))
		require.NotNil(t, got.Packfile)

		// The reader should be positioned after the "packfile\n" header
		rd := got.Packfile
		b := make([]byte, 4)
...
func TestFetchOutputEncode(t *testing.T) {
	...
	t.Run("acknowledgments with ready", func(t *testing.T) {
		...
		// Decode what we encoded — but Decode expects a full response
		// ending with packfile section, so we append one.
		pktline.WriteFlush(&buf)
	...
	t.Run("NAK when no ACKs", func(t *testing.T) {
		t.Parallel()
		resp := &FetchOutput{
			Acknowledgments: &Acknowledgments{},
		}
