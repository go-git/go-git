...
func TestReaderAtRevIndex_FromFixture(t *testing.T) {
	...
	// Load rev file content
	var revBuf bytes.Buffer
	_, err = revBuf.ReadFrom(revf)
	require.NoError(t, err)

	ri, err := NewReaderAtRevIndex(newMockRevFile(revBuf.Bytes()), crypto.SHA256.Size(), count)
	require.NoError(t, err)

	assert.Equal(t, count, ri.Count())

	// Test GetIndexPosition for all entries
	// These are the expected positions from the decoder test
	expectedPositions := []int{2, 0, 3, 4, 5, 1}
	for i, expected := range expectedPositions {
		pos, err := ri.GetIndexPosition(i)
		require.NoError(t, err)
		assert.Equal(t, expected, pos, "position %d", i)
	}

	// Test out of range
	_, err = ri.GetIndexPosition(-1)
	assert.Error(t, err)

	_, err = ri.GetIndexPosition(int(count))
	assert.Error(t, err)
	...
	for i := 0; i < int(count); i++ {
		// Get idx position from rev file
		gotIdxPos, err := ri.GetIndexPosition(i)
		require.NoError(t, err)
...
func BenchmarkReaderAtRevIndex(b *testing.B) {
	...
	b.Run("GetIndexPosition", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, _ = ri.GetIndexPosition(0)
		}
	})
}
