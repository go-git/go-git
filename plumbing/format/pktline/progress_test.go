...
func TestProgressWriter_CR_TTYErasesBeforeLineAndPreservesCR(t *testing.T) {
	t.Parallel()
	var buf bytes.Buffer
	w := pktline.NewProgressWriter(&buf, &pktline.ProgressOptions{
		Prefix: "remote: ",
	...
	want := "\x1b[Kremote: Resolving: 50%\r" + "\x1b[Kremote: Resolving: 100%\r"
	if buf.String() != want {
...
func TestProgressWriter_NewlineTTYErasePrefix(t *testing.T) {
	t.Parallel()
	var buf bytes.Buffer
	w := pktline.NewProgressWriter(&buf, &pktline.ProgressOptions{
		Prefix: "remote: ",
	...
	want := "\x1b[Kremote: Counting objects\n"
	if buf.String() != want {
		t.Fatalf("got %q\nwant %q", buf.String(), want)
	}
}
