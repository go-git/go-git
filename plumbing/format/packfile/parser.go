...
func (p *Parser) hydrateContent(oh *ObjectHeader) error {
	...
	if oh.parent != nil && oh.parent.diskType.IsDelta() {
		...
		// is not correct, go hydrate it and its parents that fall under
		// the same conditions.
		//
		...
		if oh.parent.content == nil || oh.parent.content.Len() != int(oh.parent.Size) {
			err := p.hydrateContent(oh.parent)
			if err != nil {
				return fmt.Errorf("hydrate parent at offset %v: %w", oh.Offset, err)
			}
		}
	}
...
func (p *Parser) processDelta(oh *ObjectHeader) error {
	...
	if err := p.hydrateContent(oh); err != nil {
		return err
	}
