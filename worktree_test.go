...
// TestCheckoutFastPathVerification verifies fast path by checking tree hashes
// and ensuring no worktree scanning occurs.
func (s *WorktreeSuite) TestCheckoutFastPathVerification() {
	w := &Worktree{
		r:          s.Repository,
		Filesystem: memfs.New(),
	}
	...
	currentCommit, err := w.r.CommitObject(head.Hash())
	s.NoError(err)
	currentTreeHash := currentCommit.TreeHash

	newBranch := plumbing.NewBranchReferenceName("fast-path-test")
	err = w.Checkout(&CheckoutOptions{
		Branch: newBranch,
		Create: true,
	})
	...
	s.NoError(err)
	s.Equal(newBranch, newHead.Name())

	newCommit, err := w.r.CommitObject(newHead.Hash())
	s.NoError(err)
	s.Equal(currentTreeHash, newCommit.TreeHash, "Tree hash should be identical for fast path")
}

// TestCheckoutSlowPathVerification verifies slow path is used when Force flag is set
func (s *WorktreeSuite) TestCheckoutSlowPathVerification() {
	fs := memfs.New()
	w := &Worktree{
		r:          s.Repository,
		Filesystem: fs,
	...
	err = util.WriteFile(fs, "dirty-file.txt", []byte("dirty"), 0644)
	s.NoError(err)

	err = w.Checkout(&CheckoutOptions{
		Branch: plumbing.NewBranchReferenceName("force-test"),
		Create: true,
		Force:  true,
	})
	s.NoError(err)
	...
	head, err := w.r.Head()
	s.NoError(err)
	s.Equal(plumbing.NewBranchReferenceName("force-test"), head.Name())

	_, err = fs.Stat("dirty-file.txt")
	s.Error(err, "Force checkout should have cleaned up dirty files")
}
