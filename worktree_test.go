...
func TestCheckoutParallelMatchesSequential(t *testing.T) {
	...
	wt1 := &Worktree{r: repo1, filesystem: newWorktreeFilesystem(memfs.New(), defaultProtectNTFS(), defaultProtectHFS())}
	require.NoError(t, wt1.Checkout(&CheckoutOptions{Force: true}))

	f2 := fixtures.Basic().One()
	repo2 := NewRepositoryWithEmptyWorktree(f2)
	wt2 := &Worktree{r: repo2, filesystem: newWorktreeFilesystem(memfs.New(), defaultProtectNTFS(), defaultProtectHFS())}
